...
## Canvas transform isolation (optimization lane)

- Calendar no longer replaces `Element.prototype.getBoundingClientRect`, installs capture listeners on the global mouse stream, stops native mouse events, or redispatches synthetic coordinate-scaled events. Browser and Obsidian DOM methods keep their original identity and lifecycle ownership.
- FullCalendar still has open upstream defects for calendars below transformed or zoomed ancestors ([CSS scale issue #7544](https://github.com/fullcalendar/fullcalendar/issues/7544), [coordinate issue #6825](https://github.com/fullcalendar/fullcalendar/issues/6825)) and exposes no supported coordinate-transform adapter. Calendar therefore does not attempt another inverse-scale, DOM-rewrite, or internal FullCalendar fallback.
- Canvas embeds remain available for preview, navigation, event opening, date links, identity-based drag-out, and identity-based context menus, but transformed spatial placement is explicitly not guaranteed. Position-sensitive selection-create, event move/resize, external drop, and the now indicator are disabled there with an inline explanation; open the Base in a normal tab for accurate time-grid placement and those actions. Dedicated Calendar/Base tabs and non-Canvas Markdown embeds retain their prior interaction settings.
- `scripts/test-canvas-transform-isolation.mjs` behaviorally verifies the fail-closed policy and scans the complete runtime source tree against reintroducing prototype replacement or synthetic mouse redispatch.
- 2026-07-20 optimization validation: the Canvas policy/isolation family passed 3/3, the related creation/mobile/render/zoom family passed 25/25, and the complete declared suite passed 75/75. TypeScript, diff validation, and the required separate build passed; both suite and separate builds reported `[runtime-deploy] target=none lane=optimization`. The shared nine-plugin interception guard scanned 391 runtime source files with zero violations. No optimization preview, vault reload, version, tag, release, merge, or production access occurred.
...
- Canvas embeds no longer correct FullCalendar's transformed coordinate space by intercepting native input. Spatial placement in the Canvas preview is not guaranteed; position-sensitive creation, event move/resize, external drop, and the now indicator are disabled with a visible explanation. Navigation, event opening, date links, identity-based drag-out, and identity-based context menus remain available. Inline task events still stop Canvas node context menus from replacing GCM's task-line context menu, so task property actions target the task line from Canvas embeds.
