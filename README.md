...
- Dedicated Calendar Base tabs preserve their configured day count when workspace sidebars or split panes narrow the leaf. Constrained Markdown/Canvas embeds reduce manual multi-day views to fit available width by default. Set the per-view **Embedded day count** option (`tps_preserveEmbeddedDayCount: true`) to retain an explicitly authored one-, three-, five-, or seven-day count; omitted or `false` keeps responsive reduction. Direct dashboard embeds retain their compatible `preserveDayCount` option. Ordinary manual two-through-seven-day ranges treat the selected date as their focal day, with a left-biased center for even counts; a complete standalone Week aligns to the configured first weekday and advances by seven days. When **Start on host note day** resolves an embedded Markdown host, that host date is instead the first visible column, including full seven-day and Week views. Filter-based ranges with explicit date bounds remain start-anchored and retain their exact derived span in constrained embeds.
