...
- Explicit event intervals keep FullCalendar's proportional geometry in dedicated views. Embedded time grids keep both timed cards and condensed time rows at least 18px tall, so short cards retain one readable title line and an ordinary 30-minute boundary does not become a false overlap. **Fallback event height** still applies only to entries that genuinely lack a duration or usable end; the per-view option overrides the global value, while an embed never renders any timed card below its 18px host floor. Stored intervals and the persisted `minEventHeight` key are unchanged.
