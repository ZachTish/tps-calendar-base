...
- Explicit event intervals keep FullCalendar's proportional geometry even at condensed zoom levels. **Fallback event height** is a readability floor only for entries that genuinely lack a duration or usable end; the per-view option overrides the global value and the persisted `minEventHeight` key is unchanged.
