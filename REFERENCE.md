...
Preserved from the README at 0.13.0, during the 2026-09-16 documentation cleanup. This includes earlier release notes, detailed contracts, and historical validation claims. Earlier setup steps or limitations may have been superseded. Start with [the current README](README.md), current source, and the tagged release. Relative source links remain rooted in this repository.

---

## 0.13.0 — Calendar date source

**Base view → Display → Date source** offers **Filter driven** and **Embedded/active note driven**. The existing `followActiveNoteDay` boolean values and legacy `contextDateEnabled` fallback remain readable, with the same default. No new settings page, disclosure, or unrelated control was added.

Filter driven anchors to a resolved date-filter range when that range changes; without a date range it retains ordinary saved-date/today behavior. Native filters still determine which events are included. Atomic-note mode reads the Base filter bounds for presentation without enabling synthetic formula evaluation or filtering records a second time. Embedded/active note driven prefers the containing Markdown note, then the active Markdown note; clicking the standalone/sidebar Base retains the last Markdown context. Its configured start/scheduled property supplies the date, with today when absent or invalid. Filenames no longer infer the date in this mode. A host with no date does not borrow another note's date. Manual navigation remains until the source note or its scheduled day changes. Following is event-driven, independent per rendered instance, and stops with the view. Host dates outrank filter anchoring; filters are not rewritten.

A context note need not appear in the Base result to drive the date. Scheduled updates refresh a visible sidebar even while its note editor is focused. Automatic following does not persist dates into the shared Base, preventing focus changes from becoming file modifications. Explicit calendar navigation keeps its existing persistence. Property mappings, calendar mode/day count, event creation, and note data remain unchanged.

Validation covers active-note switches, sidebar focus retention, embedded-host priority, changed/missing schedules, date-filter anchoring and unchanged manual navigation, and no automatic Base writes. Full tests and a separate production build deploy to the isolated test vault; reload Obsidian to replace existing Base instances. Computer-use verification uses synthetic notes and standalone/sidebar/embedded calendars, including narrow sidebars. Minimum Obsidian remains 1.10.0. Physical iPhone testing is not claimed. This minor feature release is for the user's BRAT pull; production is untouched.

## 0.11.1 — cleaner settings copy
...
Continuous embedded timelines use today's rendered day block and scroll within the embed, including on mobile. Returning to now cannot be interpreted as user date navigation or overridden by a pending day-centering callback. Dedicated calendars retain their existing behavior, including the 30-second inactivity timer. There are no new saved settings, note properties, or migrations; minimum Obsidian remains 1.10.0. This patch fixes the prior explicit exclusion of embedded calendars from automatic return. Validation and SHA-256 hashes are recorded in `release-notes/0.10.4.md`.

Validation on 2026-09-10: all 281 declared tests passed with zero failures or skips, including 22 new behavioral regressions for timers, range gates, cleanup, separate instances, and continuous navigation suppression. In reloaded Obsidian 1.14.1, two actual reading-mode embeds returned independently after 20 seconds; a second scroll restarted only its own deadline. A September 1 timeline stayed at its scrolled position past the deadline. Reopening the host page created a fresh scroller positioned at now. The continuous timeline stayed scrolled at 19 seconds and returned by 21.5 seconds without changing its date. Its 390 px host had a bounded 360 px scroll surface and equal 388 px client/scroll widths. Native mobile behavior is covered by component tests; the visual width check used the desktop runtime. The test run preserved plugin settings, restored temporary styles/emulation, and moved synthetic fixtures/report directly to `_archive/Calendar Now 0.10.4 QA`. Final separate build, reload, and artifact hashes are recorded in the release notes.

Page refresh here means recreating the embedded page (reload/reopen/rerender). Obsidian's ordinary query updates and the shared GCM refresh event do not identify an explicit page refresh, so they deliberately do not trigger a snap. No separate refresh command is added. Current-time positioning respects the configured visible hours.
...
- Initial host-note placement, Today, date-picker jumps, Previous/Next navigation, responsive day-count changes, and FullCalendar remounts all use the same centered selected-day contract. Even day counts retain the established earlier-middle position, and a complete Week retains configured weekday alignment.
- Explicit filter-derived ranges remain start-anchored to their exact lower bound. Host-date detection, saved selected dates, event data, and Calendar navigation persistence are unchanged.
- This is a backward-compatible presentation correction with no settings, Base-schema, command, plugin-API, or note-data migration. Minimum supported Obsidian remains 1.10.0.
...
- Editable preview keeps the Calendar visible and asks TPS Global Context Menu 1.47.0 or newer to open its responsive local Markdown editor over the originating Calendar. The editor saves body Markdown without rewriting frontmatter, supports explicit close, Escape, and outside dismissal, and becomes a keyboard-aware bottom sheet on mobile. If the capability is unavailable or declines, creation still succeeds and Calendar stays open with one concise notice.
- Every create-new route uses the same post-create policy: native records, Base toolbar creation, drag/range creation, external-event conversion, template drops, and unscheduled-note drops. Scheduling or tracking an already-existing note remains unchanged. Base toolbar note creation retains Obsidian's native Base creation behavior.
- The prior task-only boolean migrates without ambiguity: disabled becomes **Stay on Calendar**, enabled becomes **Open created item**, and the existing open behavior remains the default when no prior choice exists. No Base, note, record, or public API migration is required. Minimum supported Obsidian remains 1.10.0.
- Validation passed 44/44 focused post-create and GCM-capability checks, 34/34 adjacent formula checks, and 248/248 checks across all 20 package-declared test files, followed by the mandatory separate production build. In Obsidian 1.13.7, the real Base-toolbar SVG plus opened exactly one editable GCM card and no native Base popover, preserved complex Markdown after dismissal, kept Calendar active for Preview and Stay, and activated the created note for Open. All fixtures were moved directly to `_archive` with `scheduled` removed; Calendar runtime settings and the original layout were restored. Editable preview deliberately requires the connected GCM 1.47.0 capability and does not substitute a read-only or third-party hover surface when unavailable.

## 0.8.4

- Every timed event now has an 18px FullCalendar geometry floor in dedicated, mobile, embedded, and continuous time-grid views. Very short records keep their exact visual start position while only the rendered bottom extends far enough to show one title line.
- The floor is presentation-only. Calendar still supplies and persists the real start, end, and duration, and drag, drop, resize, filtering, all-day rows, and month views retain their existing data semantics. FullCalendar includes the enlarged visual span in overlap layout and hit testing instead of letting a CSS-only card cover neighboring events.
- Dedicated calendars retain their configured zoom and time-row density. Embedded calendars retain their existing 18px condensed-row safeguard. The persisted `minEventHeight` setting remains the separate fallback for records that genuinely lack a usable duration or end; no setting, Base, or record migration is required. Minimum supported Obsidian remains 1.10.0.
- Validation passed 68/68 focused geometry/mobile checks and 245/245 checks across all 20 package-declared test files, followed by the required separate production build. After an app reload in isolated Obsidian 1.13.7, a real five-minute record in the dedicated three-day Calendar rendered an 18px harness with a visible title while its top stayed aligned to the 09:00 slot and its stored duration remained five minutes; a natural 30-minute record remained 21px tall. The same record retained its 18px harness and visible title under mobile emulation. The temporary fixture was moved directly to `_archive` with its schedule removed, desktop mode and the original Calendar view were restored, runtime `data.json` remained absent, `.hotreload` remained unchanged, and production was not accessed.

## 0.8.3

- **Start on host note day → Use host note date** now resolves the containing Markdown or Daily Note even when the embedded Base controller identifies only its `.base` file or the host attaches after initial configuration. Calendar retries at its first runnable update, reads configured host frontmatter first, and falls back to the Daily Note filename.
- Embedded multi-day calendars now place the host date in the first visible column, including full seven-day and Week views, instead of centering the range several days before it. Standalone calendars retain their centered focal-day behavior, and later navigation inside the same host note remains authoritative.
- Markdown-rendered Calendar embeds pass their source note directly. Custom direct-render API callers may provide the additive optional `hostNotePath`; an embed without any discoverable Markdown context retains the normal saved-date fallback. Existing settings, saved dates, Base definitions, notes, commands, and minimum supported Obsidian 1.10.0 require no migration.
- Validation passed 45/45 focused Calendar lifecycle and direct-render checks plus 245/245 checks across all 20 package-declared test files. The required separate production build deployed byte-identical artifacts to the isolated test vault. Obsidian 1.13.7 opened a seven-day Base saved to August 10 inside the August 28 Daily Note with August 28 as its first visible column, re-anchored the same Base to August 29 in the next host note, and kept the standalone August 19 three-day view centered on August 18–20. The temporary fixtures were moved directly to `_archive` with their schedules removed, the original standalone view was restored, Calendar's runtime `data.json` remained absent, `.hotreload` remained unchanged, and production was not accessed.

## 0.8.2

- Embedded time-grid calendars now give timed event cards and condensed time rows an 18px host-level visual floor, so real 15- and 30-minute events remain readable in short Daily Note embeds without making an ordinary 30-minute event collide with the next 30-minute interval. The floor changes only FullCalendar's host geometry: stored start/end values, drag and resize data, and dedicated calendar proportions remain unchanged. In embeds, it also prevents a lower or zero **Fallback event height** from shrinking a missing-duration card below 18px.
- Validation passed 72/72 focused embedded-geometry, direct-render, and mobile interaction checks plus all 20 package-declared test files. The required separate production build deployed byte-identical artifacts to the isolated test vault. Obsidian 1.13.7 then rendered a real 15-minute card with an 18px harness and visible title, kept the adjacent 30-minute card full-width, and split only the two events that truly shared a start time; the dedicated view retained proportional geometry. The temporary fixture and UI state were restored, TishOS v0.2 was not modified, and production was not accessed.
...
- Existing views remain on **Fit available width**, preserving responsive day reduction. Dedicated Calendar tabs and filter-derived exact ranges keep their existing behavior.
- The direct dashboard renderer's `preserveDayCount` option remains compatible and independent from the per-view choice, including live true-to-false view edits.
- This is a backward-compatible opt-in view feature with no automatic Base or settings migration. Minimum supported Obsidian remains 1.10.0.
...
- Daily Note detection accepts both the canonical note for the scheduled date and an explicitly targeted note identified by `kind: dailynote` or `#dailynote`. If `## Scheduled` is missing, Calendar creates it without rewriting existing sections.
- Explicit dedicated task target notes retain their existing append behavior. No existing task, Daily Note, section, setting, Base definition, or plugin API is migrated or rewritten.
...
- The receiver now gives an app-switched mobile vault a bounded opportunity to materialize the exact `.base` file and named Calendar view. Every attempt rereads the adapter-backed file and repeats the same strict vault/path/type/name checks, so a stale Obsidian metadata cache cannot authorize a different target.
- A widget's exact transient date can be shown even when the Base has explicit manual-navigation bounds. The date remains navigation-only and non-persisting; normal in-Obsidian Previous, Today, Next, and date-picker navigation still obey the Base bounds.
- Focused regressions cover delayed Base materialization, cancellation, current-leaf tracking, and exact out-of-bounds transient focus. Existing settings, commands, Base files, stored dates, and minimum Obsidian 1.10.0 remain unchanged.
...
- Duration-mode views accept literal or formula task durations without allowing same-named note frontmatter to shadow the task value. End-datetime views use a valid row end first and fall back to the task's canonical `timeEstimate` when no usable end is present.
- Resizing a timed task always keeps canonical `timeEstimate` minutes current. A distinct configured duration field receives the same minutes, while a configured end-datetime field receives the formatted end timestamp; Calendar no longer writes a minute count into a datetime field. Moving a task to all-day removes both duration carriers and records its configured all-day flag; moving it back to a timed slot removes that flag so it cannot snap back.
- FullCalendar receives each exact start/end interval unchanged. Explicit intervals are no longer enlarged by Calendar's event-card minimum height, so 15-, 30-, 60-, and 90-minute tasks remain visually proportional at condensed zoom levels.
- The existing `minEventHeight` key is now honored per view and is labeled **Fallback event height** in settings and Base view options. It applies only when an event has no duration or usable end, supports an explicit zero, and introduces no persisted-settings migration.
- This backward-compatible correctness patch changes no task syntax, commands, API versions, Base schema, or minimum supported Obsidian version. Existing views and settings continue to work; minimum supported Obsidian remains 1.10.0.

## 0.4.3

- Inline-task status display and Base-filter task scheduling now consume only GCM's exact version-1 `taskCheckboxes` capability. Calendar no longer carries a private todo/complete/working/holding/cancelled marker table or reaches through a status-service fallback.
- Custom one-character markers, aliases, alternate working markers, and uppercase `X` normalization follow GCM's ordered snapshot. A valid one-character checkbox remains structurally a task even when its marker is unmapped, so its Kind, title, tags, authored inline fields, and custom formulas remain usable; derived `checkboxStatus`, open, done, and completed aliases are absent instead of guessed.
- Dragging a task onto Calendar captures both the exact source revision and resolved checkbox marker. After confirmation, Calendar revalidates the mapping and follows only one uniquely shifted unchanged line; a changed mapping, edited source, malformed marker, unmapped marker, or ambiguous duplicate performs no write. The atomic line patch applies scheduling metadata, clears only checkbox-owned workflow-status fields, and preserves a configured relational Status field.
- Task and dedicated-note creation resolves the effective status before creating a target file or appending to a note, then revalidates it inside the append callback. The checkbox is the task line's sole persisted workflow carrier, so Calendar no longer writes a redundant `[status:: ...]`; dedicated task-note frontmatter uses GCM's workflow key and never overwrites a relational Status property. Missing, stale, malformed, or changed mappings perform no task-line write.
- Base/formula namespaces now remain distinct: authored `row.status` stays readable as row metadata, while `task.status` and `task.checkboxStatus` resolve only from a mapped checkbox. Rescheduling also revalidates the current marker inside the atomic source update.
- Inline-task parsing accepts only Obsidian's one-code-unit checkbox syntax. Empty, multi-character, or astral tokens are no longer promoted to `[ ]`; existing unmapped tasks remain visible but task-note/menu and workflow-mutation routes require a currently mapped token, and mapped event icons use the provider's configured icon instead of a copied marker table.
- A mapped checkbox status remains readable when GCM cannot authoritatively classify whether it is done, but Calendar omits `open`/`done`/`completed` aliases, refuses completion filters for that task, and leaves the event active-looking. Native note and external-event status styling retains Calendar's standalone active/inactive-list behavior.
- This backward-compatible reliability patch changes no settings, commands, Base definitions, note/task syntax, plugin API versions, or stored data. Minimum supported Obsidian remains 1.10.0 and GCM 1.17.1 is required for synthesized status behavior.
...
- Today, date-picker jumps, Previous/Next, reloads, FullCalendar remounts, and responsive partial-week layouts all preserve that same selected first day.
- Automatic filter-based ranges with explicit date bounds remain anchored to their lower bound, while complete seven-day weeks still align to the configured first weekday.
- This is a backward-compatible correctness patch with no settings, Base-schema, command, plugin-API, or note-data migration. Minimum supported Obsidian remains 1.10.0.
...
- Automatic filter-based ranges with explicit date bounds remain start-anchored, so a July 31–August 5 filter still renders those exact six columns. Manually configured views stay centered even when filtered, and full seven-column weeks still align to the configured first weekday.
...
- A full seven-column week still aligns to the configured first day of the week and Previous/Next advances exactly seven local calendar days. When a constrained embed responsively shows fewer week columns, it begins on the selected day so Today and date-picker targets remain visible.
- Explicit filter date bounds keep their exact inclusive range, while data-derived filter-based views recalculate from `2d` through `7d` or month whenever matching entries change without moving the user's selected day.
- Date navigation now stays within explicit bounds; month navigation starts at the intended adjacent month even from dates such as January 31. Delayed saved-date writes snapshot the day when navigation occurs.
- This is a backward-compatible correctness patch with no settings, Base-schema, command, plugin-API, or note-data migration. Existing manual multi-day `tps_currentDate` values are now interpreted as the first visible day rather than a centered day, so the visible range intentionally shifts forward on first load. Minimum supported Obsidian remains 1.10.0.
- Validation passed all 134 tests, three containment checks, TypeScript, the required separate production-mode build, and reloaded Obsidian 1.12.7 test-vault QA. Manual three-day, exact two/six-day, Monday-aligned week, seven-day navigation, and constrained two-column week flows rendered their expected dates; production was not accessed.
...
- Filter-based views now preserve the actual Boolean filter tree: AND ranges intersect, OR branches span their union, equality is one day, strict upper bounds exclude their boundary, and one-sided ranges use matching-entry dates for the missing side. Non-range conditions such as `scheduled is not empty`, NOT complements, unresolved expressions, and contradictory bounds no longer create a false one-day navigation lock.
- Relative whole days, weeks, and months now use local calendar arithmetic across daylight-saving transitions; month arithmetic clamps safely at month-end. Hours, minutes, and fractional calendar units retain elapsed-time behavior.
- The visible `tps_viewMode` Base option is authoritative over legacy aliases. Responsive time-grid layouts and display-setting remounts recalculate their rendered start from the latest semantic anchor without replaying stale parent or jump props, and the date picker now moves FullCalendar immediately.
- **Start on host note day** is once again host-scoped: an unrelated active Markdown note can no longer overwrite the embedded calendar after its delayed initial render. Explicit API/date-picker navigation remains authoritative, followed by the host note and then saved state.
- Newly persisted view dates use a timezone-stable local `YYYY-MM-DD` key; existing ISO timestamps remain readable. This backward-compatible patch has no settings or note-data migration and keeps minimum supported Obsidian at 1.10.0.
...
- 2026-07-18 contained-workspace validation: the current view-setting source changes were preserved during relocation; the declared suite passed 73/73, including the three new workspace/path guards, and a standalone production-mode build reported `[runtime-deploy] target=test ... unchanged`. The isolated app was reloaded with `Reload app without saving`, and its file explorer showed `Plugin Development` with all nine source repositories and shared tools. No live promotion was requested.
...
Release `0.4.4` is self-contained for fresh BRAT installs: the build combines `main.css` and `styles-ui.css` into the standard release `styles.css`, so runtime styling does not depend on an extra file BRAT does not download. `styles-ui.css` remains a maintained build input and legacy deployment artifact.
...
- Unknown fields, unsupported versions, control characters, traversal, link-fragment/alias delimiters, oversized requests, a wrong vault, a missing or duplicate named Calendar view, and an uninitialized/ambiguous renderer fail closed. The protocol accepts no command ID, note content, mutation, or arbitrary callback URL. A valid requested day is allowed outside manual Base navigation bounds only for this transient external focus; ordinary Calendar controls remain bounded.
...
- Timed task intervals are row-owned. In duration mode, Calendar uses the configured task-row duration before canonical `timeEstimate`; in end mode, it uses a valid configured row end before `timeEstimate`. Containing-note frontmatter cannot replace or supply a child task's duration. Only a task with no usable interval receives the configured fallback event height.
...
- For task creation, `taskCreateDestination` chooses sink behavior:
  - `daily-note` — place the task line in the Daily Note's exact `## Scheduled` section, creating the section when absent and ordering scheduled tasks future-first.
...
- `After creating an item` applies one policy to note and task creation: **Editable preview over Calendar**, **Open created item**, or **Stay on Calendar**. Task mode previews or opens the exact destination file that received the task, which may be the scheduled Daily Note, a configured shared target, or a dedicated task note. Editable preview requires GCM UI API v1 (GCM 1.47.0 or newer); failure leaves the created item intact and keeps Calendar active.
...
- In Settings set:
  ...
  - `After creating an item` to `Editable preview over Calendar`, `Open created item`, or `Stay on Calendar`.
...
- Validated combinations:
  ...
  - No filter `task.path` + plugin `taskCreateTargetPath` set -> task-mode creation writes into the configured plugin target note and applies the selected post-create behavior to that destination.
...
- Dedicated Calendar Base tabs preserve their configured day count when workspace sidebars or split panes narrow the leaf. Constrained Markdown/Canvas embeds reduce manual multi-day views to fit available width by default. Set the per-view **Embedded day count** option (`tps_preserveEmbeddedDayCount: true`) to retain an explicitly authored one-, three-, five-, or seven-day count; omitted or `false` keeps responsive reduction. Direct dashboard embeds retain their compatible `preserveDayCount` option. Ordinary manual two-through-seven-day ranges treat the selected date as their focal day, with a left-biased center for even counts. **Start on host note day** supplies that same selected focal day to embedded Markdown calendars rather than changing its placement. A complete Week aligns to the configured first weekday and advances by seven days. Filter-based ranges with explicit date bounds remain start-anchored and retain their exact derived span in constrained embeds.
...
- Non-active/completed events remain muted in both dedicated and embedded views. Native notes and external events use Calendar's configured/GCM active-status list. Inline tasks are muted only when GCM's authoritative `isDoneStatus` classifier returns true; a missing, throwing, or non-boolean classifier leaves completion unknown and cannot borrow Calendar's standalone inactive-status fallback. The Calendar setting labeled `Completed Event Opacity` controls opacity, defaulting to 50%; colored non-active events are also desaturated and darkened. Inline task workflow state comes only from GCM's checkbox mapping; an authored `row.status` remains ordinary row metadata and cannot override `task.status`.
...
- Every timed event harness keeps an 18px visual floor across dedicated, mobile, embedded, and continuous time grids. FullCalendar anchors the top to the exact event start and extends only the rendered bottom when the real interval would be shorter, while stored intervals and drag/resize payloads remain exact. Dedicated calendars retain their configured time-row density; embedded time grids additionally keep condensed rows at least 18px tall so an ordinary 30-minute boundary does not become a false overlap. **Fallback event height** still applies only to entries that genuinely lack a duration or usable end, and the persisted `minEventHeight` key is unchanged.
...
- Filter-based views recalculate independently when the active Base view changes and whenever the occupied span of matching entries changes. Entry-derived recalculation changes the concrete day count without moving the selected day; an explicit bounded range starts on its lower bound. All-day and timed intervals are half-open, so an event ending exactly at midnight does not add the next day.
...
- **Rules & creation** is the default page. It keeps concise Base-filter guidance, initial note/task creation mode, the always-visible post-create dropdown, and dependent task destination/path controls together. Longer Base examples are contained in one optional reference disclosure.
...
- 2026-08-15 (0.5.1) mobile widget-handoff validation: all 226 declared tests passed, including delayed Base materialization, current-leaf replacement, stale-request cancellation, renderer settlement, and exact transient focus outside manual navigation bounds. The required separate production build reported the isolated test runtime unchanged after the suite deployed `main.js`. Source and runtime `main.js` matched SHA-256 `652562d31c12320298aee113c03ebd175da961df76160153f6174d726af12390`. After `Reload app without saving` in isolated Obsidian 1.13.7, the exact version-1 URI for `_archive/Calendar Day Bounds QA.base#Strict one-day filter QA` visibly opened August 15 even though the Base is explicitly filtered and saved to August 7. The failure notice did not recur, and `tps_currentDate` remained `2026-08-07`, confirming navigation-only presentation. Production was not accessed.
...
- 2026-07-04: Calendar task creation began appending inline task lines to the end of resolved target notes instead of inserting immediately after frontmatter. Version 0.5.4 narrows that legacy behavior to explicit dedicated targets; Daily Notes now use their exact `## Scheduled` section so tasks cannot inherit a trailing unrelated heading. Validation for the historical change used focused creation tests and a production build.
...
- 2026-07-04: Live Obsidian UI smoke-tested a disposable Calendar Base toolbar `+ New` task creation flow with `initialCreateMode: task`, `taskCreateDestination: daily-note`, and an active Base `task.path == "Inbox/TPS Calendar Base UI QA Target.md"` filter. The modal wrote the inline task to the resolved target note, refreshed the calendar while the legacy open-after-create toggle was disabled, and clicking the created calendar event opened the inline-task chooser; `Open source task line` focused the exact source note. Version 0.9.0 supersedes that toggle with the shared three-way post-create dropdown. The disposable QA files were moved to `Archive/` after validation.
