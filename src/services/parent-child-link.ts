...
import {
  ...
  canGcmAutomaticallyMutateFrontmatter,
...
function findFrontmatterKeyCaseInsensitive(
  target: Record<string, any>,
  key: string,
): string | null {
  const normalized = normalizeFrontmatterKey(key);
  if (!normalized) return null;
  const direct = Object.keys(target || {}).find(
    (candidate) => normalizeFrontmatterKey(candidate) === normalized,
  );
  return direct ?? null;
}
...
function resolveLinkTargetForSource(
  app: App,
  targetFile: TFile,
  sourcePath: string,
): string {
  ...
  const candidate =
    extractLinkTarget(generated) ??
    app.metadataCache.fileToLinktext(targetFile, sourcePath, true) ??
    targetFile.path;
  return (
    normalizeLinkTarget(candidate) ??
    normalizeLinkTarget(targetFile.path) ??
    targetFile.path
  );
}
...
async function tagParentAfterChildLink(
  app: App,
  parentFile: TFile,
): Promise<void> {
  ...
  await app.fileManager.processFrontMatter(parentFile, (fm) => {
    ...
    logger.log(
      `[ParentChildLink] Tagged parent note with ${tagsToAdd.map((tag) => `#${tag}`).join(", ")}`,
    );
  });
}

async function ensureParentSelfLink(
  app: App,
  parentFile: TFile,
  ...
  await app.fileManager.processFrontMatter(parentFile, (fm) => {
    ...
    if (
      values.some((value) =>
        linkReferencesFile(app, value, parentFile.path, parentFile),
      )
    )
      return;
    setFrontmatterValueCaseInsensitive(fm, parentKey, [...values, selfLink]);
    logger.log(
      `[ParentChildLink] Added self parent link to ${parentFile.path}: ${parentKey} = ${selfLink}`,
    );
  });
}
...
export async function applyParentLinkToChild(
  ...
  await app.fileManager.processFrontMatter(childFile, (fm) => {
    ...
    logger.log(
      `[ParentChildLink] Added parent link to ${childFile.path}: ${parentKey} = ${parentLink}`,
    );
  });
...
export async function createBidirectionalLink(
  ...
  try {
    ...
    await app.fileManager.processFrontMatter(parentFile, (fm) => {
      ...
      if (
        !children.some((existing) =>
          linkReferencesFile(app, existing, parentFile.path, childFile),
        )
      ) {
        children.push(childLink);
        setFrontmatterValueCaseInsensitive(fm, childKey, children);
        logger.log(
          `[ParentChildLink] Added child link to ${parentFile.path}: ${childKey} now has ${children.length} items`,
        );
      } else {
        logger.log(
          `[ParentChildLink] Child link already exists in ${parentFile.path}`,
        );
      }
    });

    logger.log(
      `[ParentChildLink] ✓ Bidirectional link created: ${childFile.basename} ↔ ${parentFile.basename}`,
    );
  } catch (error) {
    logger.error(
      `[ParentChildLink] Failed to create bidirectional link:`,
      error,
    );
    throw error;
  }
...
export async function removeBidirectionalLink(
  ...
  try {
    ...
    await app.fileManager.processFrontMatter(childFile, (fm) => {
      if (getFrontmatterValueCaseInsensitive(fm, parentLinkKey) === undefined)
        return;
      deleteFrontmatterValueCaseInsensitive(fm, parentLinkKey);
      logger.log(
        `[ParentChildLink] Removed parent link from ${childFile.path}`,
      );
    });
    ...
    await app.fileManager.processFrontMatter(parentFile, (fm) => {
      ...
      if (Array.isArray(existingRaw)) {
        const filtered = existingRaw.filter(
          (link: any) =>
            !linkReferencesFile(app, link, parentFile.path, childFile),
        );
        ...
        logger.log(
          `[ParentChildLink] Removed child link from ${parentFile.path}`,
        );
        return;
      }
    ...
    logger.log(
      `[ParentChildLink] ✓ Bidirectional link removed: ${childFile.basename} ↔ ${parentFile.basename}`,
    );
  } catch (error) {
    logger.error(
      `[ParentChildLink] Failed to remove bidirectional link:`,
      error,
    );
    throw error;
  }
...
export async function removeChildLinkFromParent(
  ...
  try {
    const matchContext = createDeletedMarkdownLinkContext(
      deletedPath,
      remainingMarkdownPaths,
    );
    if (!matchContext)
      return { removedReferences, preservedAmbiguousReferences };
    const mutationPermission = await canGcmAutomaticallyMutateFile(
      app,
      parentFile,
    );
    if (mutationPermission === false) {
      logger.log(
        `[ParentChildLink] Skipped automatic detached-link cleanup for template ${parentFile.path}`,
      );
      return { removedReferences, preservedAmbiguousReferences };
    }
    const shouldRemove = (value: unknown): boolean => {
      ...
      if (
        resolvedFile &&
        app.vault.getAbstractFileByPath(resolvedFile.path) instanceof TFile
      )
        return false;
      const decision = classifyDeletedMarkdownLink(
        value,
        parentFile.path,
        matchContext,
      );
    ...
    await app.fileManager.processFrontMatter(parentFile, (fm) => {
      if (canGcmAutomaticallyMutateFrontmatter(app, fm) === false) return;
      const existingRaw = getFrontmatterValueCaseInsensitive(fm, childLinkKey);
      if (Array.isArray(existingRaw)) {
        const filtered = existingRaw.filter(
          (link: unknown) => !shouldRemove(link),
        );
        if (filtered.length !== existingRaw.length) {
          ...
          logger.log(
            `[ParentChildLink] Removed detached child link '${deletedPath}' from ${parentFile.path}`,
          );
        }
        return;
      }
    ...
    logger.error(
      `[ParentChildLink] Failed to remove child link from parent:`,
      error,
    );
    throw error;
  }
